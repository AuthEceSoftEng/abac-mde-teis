...
public class GetaccountproductHandler{
    ...
    public GetaccountproductHandler(String authHeader, int accountId, int productId, UriInfo oApplicationUri){
    ...
    public JavaproductModel createHypermedia(JavaproductModel oJavaproductModel){
		if (this.bIsClientAuthenticated == true){ //if the user is authenticated apply ABAC checks
				...
				if (pdp.getPermission(oAuthenticationAccount, "product", Integer.toString(oJavaaccountModel.getaccountId()), null, Action.POST).equals(AuthorizationResultCode.PERMIT)){
		        ...
		        oJavaproductModel.getlinklist().add(new HypermediaLink(String.format("%s%s", oApplicationUri.getBaseUri(), String.format("%s", this.strResourcePath).replaceAll("multiproduct","multiproductManager")).substring(0, iLastSlashIndex), "Get all product of this account", "GET", "Parent"));
